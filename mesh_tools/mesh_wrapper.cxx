...
void MeshWrapper::readFromInp(std::ifstream &fin)
{
	/*
		读入.inp文件
		*/
	...
	std::regex node_format("^([\\d\\.Ee\\s]+),([\\d\\.Ee\\s]+),([\\d\\.Ee\\s]+),([\\d\\.Ee\\s]+)$");
	std::regex element_format(
		"^([\\d\\.Ee\\s]+),([\\d\\.Ee\\s]+),([\\d\\.Ee\\s]+),([\\d\\.Ee\\s]+),([\\d\\.Ee\\s]+)$");
	...
	//插入网格点
			...
			else if (element_section)
			{
				...
				int v[4];
#ifdef __linux
				sscanf(line_str.c_str(), "%d, %d ,%d , %d , %d", &id, v, v + 1, v + 2, v + 3);
#endif // __linux
#ifdef _WIN64
				sscanf_s(line_str.c_str(), "%d, %d ,%d , %d , %d", &id, v, v + 1, v + 2, v + 3);
#endif // _WIN64 \
	//int转换为handle
...
void MeshWrapper::readFromOvm(std::ifstream &fin)
{
	OpenVolumeMesh::IO::FileManager manager;
	manager.readStream(fin, *ovm_mesh);
}

void MeshWrapper::saveToOVM(std::string filename)
{
	OpenVolumeMesh::IO::FileManager manager;
...
// void MeshWrapper::faceData(VMeshPtr mesh, std::vector<double[3]> &points, std::vector<int[3]> &faces)
// {
...
bool MeshWrapper::isSameHalfface(const std::vector<int> &f1, const std::vector<int> &f2)
{
...
void MeshWrapper::addCell(std::vector<OvmVeH> &v, std::map<TF, OvmFaH> &faces)
{
	/*
		变量定义
		*/
...
void MeshWrapper::readMesh(std::string filename)
{
